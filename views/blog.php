<div x-show="page==='blog'" x-cloak class="fade-in">
  <div class="flex items-center gap-4 mb-8">
    <h2 class="text-2xl font-black text-mt-brown uppercase tracking-tighter">Blog</h2>
  ...
  <div x-show="!blogPost" class="grid md:grid-cols-3 gap-6">
    <template x-for="post in blogPosts" :key="post.id">
      <div @click="abrirPost(post.slug)"
        ...
        <img :src="post.imagen_portada||'https://images.unsplash.com/photo-1548199973-03cce0bbc87b?w=600'"
        ...
        <div class="p-5">
          ...
          <span class="mt-3 inline-block text-mt-orange font-black text-xs uppercase tracking-widest">Leer mas →</span>
        </div>
      </div>
    </template>
    <div x-show="blogPosts.length===0" class="col-span-3 text-center py-16 text-slate-400 italic">
  ...
  <div x-show="blogPost" class="max-w-3xl mx-auto">
    <button @click="blogPost=null" class="flex items-center gap-2 text-mt-brown font-bold text-sm mb-6 hover:text-mt-orange transition-colors">
      <i class="fas fa-arrow-left"></i> Volver al blog
    </button>
    <template x-if="blogPost">
      <article class="bg-white rounded-[2rem] border border-mt-cream overflow-hidden">
        <img :src="blogPost.imagen_portada||'https://images.unsplash.com/photo-1548199973-03cce0bbc87b?w=800'"
